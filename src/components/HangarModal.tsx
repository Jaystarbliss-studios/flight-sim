...
export const HangarModal: React.FC<HangarModalProps> = ({ currentAircraft, onSelect, onClose }) => {
  const [selectedId, setSelectedId] = useState(currentAircraft.id);

  const confirm = () => {
    const aircraft = AIRCRAFTS.find((item) => item.id === selectedId) ?? AIRCRAFTS[0];
    onSelect(aircraft);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-lg p-4 select-none">
      <div className="w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950/95 shadow-2xl text-white">
        <div className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-800 bg-slate-950/95 px-5 py-4 backdrop-blur-md">
          <div>
            <div className="flex items-center gap-2 text-cyan-300 text-xs font-mono font-bold uppercase tracking-[0.2em]">
              <Plane className="w-4 h-4" /> Hangar
            </div>
            <h2 className="mt-1 text-xl sm:text-2xl font-black">Choose Your Aircraft</h2>
            <p className="text-xs text-slate-400 mt-1">Select the aircraft before dispatch. The simulator will not silently replace your choice with another model.</p>
          </div>
          <button onClick={onClose} className="rounded-full p-2 bg-slate-900 border border-slate-800 text-slate-400 hover:text-white" title="Close hangar">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {AIRCRAFTS.map((aircraft) => {
            ...
            return (
              <button
                ...
                className={`group relative overflow-hidden rounded-2xl border text-left transition-all ${active ? 'border-cyan-400 bg-cyan-950/50 shadow-xl shadow-cyan-500/10' : 'border-slate-800 bg-slate-900/70 hover:border-slate-600'}`}
              >
                <div className="h-36 relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-cyan-950/40">
                  ...
                  <div className="absolute inset-x-8 top-1/2 h-2 rounded-full bg-slate-200/90 shadow-[0_0_20px_rgba(255,255,255,.18)] rotate-[-5deg]" />
        ...
        <div className="flex flex-col sm:flex-row gap-3 border-t border-slate-800 p-5">
          ...
          <button onClick={confirm} className="flex-1 rounded-xl bg-cyan-600 hover:bg-cyan-500 py-3 text-sm font-black shadow-lg shadow-cyan-600/20">Use {AIRCRAFTS.find((item) => item.id === selectedId)?.name ?? 'Aircraft'}</button>
        </div>
      </div>
    </div>
  );
