...
const HANGAR_PREF_KEY = 'flight-sim-hangar-preference';

interface HangarPreference { aircraftId: string; visualModel: VisualModel; }
export function readHangarPreference(): HangarPreference | null {
  try {
    const raw = localStorage.getItem(HANGAR_PREF_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as HangarPreference;
    return AIRCRAFTS.some((a) => a.id === parsed.aircraftId) ? parsed : null;
  } catch { return null; }
}
...
export const HangarModal: React.FC<HangarModalProps> = ({ currentAircraft, currentVisualModel, onSelect, onClose }) => {
  ...
  const confirm = () => {
    const chosenModel = detailedAvailable ? visualModel : 'procedural';
    try { localStorage.setItem(HANGAR_PREF_KEY, JSON.stringify({ aircraftId: selectedAircraft.id, visualModel: chosenModel })); } catch { /* storage is optional */ }
    onSelect(selectedAircraft, chosenModel);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-lg p-4 select-none">
      <div className="w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950/95 shadow-2xl text-white">
        ...
        <div className="px-5 pb-5"><div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><div className="flex items-center justify-between gap-3 mb-3"><div><div className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400">Aircraft Visual</div><div className="text-sm font-bold mt-1">How should {selectedAircraft.name} appear?</div></div><span className="text-[10px] font-mono text-slate-500">{detailedAvailable ? 'A320 detailed asset available' : 'Detailed asset not yet mapped'}</span></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button disabled={!detailedAvailable} onClick={() => setVisualModel('procedural')} className={`rounded-xl border p-3 text-left ${visualModel === 'procedural' || !detailedAvailable ? 'border-cyan-500 bg-cyan-950/40' : 'border-slate-800 bg-slate-950/60'} ${!detailedAvailable ? 'opacity-70' : ''}`}><div className="flex items-center gap-2 font-bold text-sm"><Box className="w-4 h-4 text-cyan-300" /> Simulator Model</div><div className="text-[11px] text-slate-400 mt-1">Native procedural aircraft with controlled flight-surface animation and no network dependency.</div></button>
          <button disabled={!detailedAvailable} onClick={() => setVisualModel('detailed')} className={`rounded-xl border p-3 text-left ${visualModel === 'detailed' && detailedAvailable ? 'border-emerald-500 bg-emerald-950/40' : 'border-slate-800 bg-slate-950/60'} ${!detailedAvailable ? 'opacity-50 cursor-not-allowed' : ''}`}><div className="flex items-center gap-2 font-bold text-sm"><Sparkles className="w-4 h-4 text-emerald-300" /> Detailed A320</div><div className="text-[11px] text-slate-400 mt-1">High-detail external A320 asset, shown only when explicitly selected.</div></button>
        </div></div></div>
        <div className="flex flex-col sm:flex-row gap-3 border-t border-slate-800 p-5"><button onClick={onClose} className="sm:w-40 rounded-xl border border-slate-700 bg-slate-900 py-3 text-sm font-bold text-slate-300 hover:text-white">Cancel</button><button onClick={confirm} className="flex-1 rounded-xl bg-cyan-600 hover:bg-cyan-500 py-3 text-sm font-black shadow-lg shadow-cyan-600/20">Use {selectedAircraft.name}</button></div>
      </div>
    </div>
  );
};
