...
export class FlightPhysics {
  ...
  public initFlightState(runwayAltMeters: number = 38): FlightState {
    ...
    return {
      ...
      safetyViolations: [],
      isTacticalHud: Boolean(this.aircraft.isFighter),
      afterburnerActive: false,
      flaresRemaining: this.aircraft.isFighter ? 30 : 0,
      lastFlareTime: 0,
      crewRole: 'PF',
      isVirtualCopilotActive: !this.aircraft.isFighter,
      targetDrone: {
        x: this.plan.destination.worldX * 0.4,
        y: 4200,
        z: this.plan.destination.worldZ * 0.4,
        distanceNm: 22,
        bearingDeg: 250,
        locked: false,
      },
  ...
  public update(state: FlightState, deltaSec: number, groundElevationM: number, destX: number, destZ: number): FlightState {
    ...
    if (state.enginesRunning) {
      ...
      const abBurnFactor = isAb ? 2.8 : 1.0;
      const hourlyBurn = (this.aircraft.burnRateKgPerHour * (0.3 + 0.7 * baseThrust) * abBurnFactor) / 3600;
    ...
    if (onGround) {
      ...
      if (this.aircraft.isFighter) {
        const fighterG = (liftForce / weightForce) + state.pitchInput * 4.0;
        state.gForce = Math.max(-3.0, Math.min(9.0, fighterG));
      } else {
    ...
    // Tactical Target Drone Tracking (inspired by dimartarmizi/web-flight-simulator)
    const droneX = state.targetDrone?.x ?? (destX * 0.4);
    const droneZ = state.targetDrone?.z ?? (destZ * 0.4);
    const droneDistM = Math.hypot(droneX - state.x, droneZ - state.z);
    const droneDistNm = Number((droneDistM / 1852).toFixed(1));
    const droneBearing = ((Math.atan2(droneX - state.x, -(droneZ - state.z)) * 180) / Math.PI + 360) % 360;
    let droneBearingDiff = Math.abs(droneBearing - state.headingDeg);
    if (droneBearingDiff > 180) droneBearingDiff = 360 - droneBearingDiff;
    const locked = droneBearingDiff < 14 && droneDistNm < 30;

    state.targetDrone = {
      x: droneX,
      y: 4200,
      z: droneZ,
      distanceNm: droneDistNm,
      bearingDeg: Math.round(droneBearing),
      locked,
    };
