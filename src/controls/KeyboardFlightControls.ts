...
/** Desktop keyboard layer. Input is state-only; physics remains responsible for aircraft response. */
export class KeyboardFlightControls {
  ...
  constructor(getState: () => FlightState | null, onCamera: (mode: CameraMode) => void) { this.getState = getState; this.onCamera = onCamera; }
  public attach() { if (this.attached) return; this.attached = true; window.addEventListener('keydown', this.handleKeyDown); window.addEventListener('keyup', this.handleKeyUp); window.addEventListener('blur', this.handleBlur); }
  public detach() { if (!this.attached) return; this.attached = false; window.removeEventListener('keydown', this.handleKeyDown); window.removeEventListener('keyup', this.handleKeyUp); window.removeEventListener('blur', this.handleBlur); this.handleBlur(); }

  private handleKeyDown = (event: KeyboardEvent) => {
    const target = event.target as HTMLElement | null;
    if (target?.matches('input, textarea, select, [contenteditable="true"]')) return;
    const key = event.key.toLowerCase();
    const handled = ['w','s','a','d','q','e','b','g','f','r','p','o','x','l','n','k','t','z','c','1','2','3','4','escape',' ','+','=','-','_'].includes(key)
    ...
    switch (key) {
      ...
      case 'r': state.flapsIndex = Math.max(0, state.flapsIndex - 1); break;
      case 'b': state.brakesActive = true; break;
      case 'q': state.yawInput = -1; break;
      case 'e': state.yawInput = 1; break;
      case 'o': state.autopilotEnabled = !state.autopilotEnabled; state.autoThrottleEnabled = state.autopilotEnabled; break;
      case 'x':
        state.spoilersDeployed = !state.spoilersDeployed;
        if (state.flaresRemaining && state.flaresRemaining > 0) {
          state.flaresRemaining--;
          state.lastFlareTime = performance.now();
      ...
      case '+': case '=': state.throttle = Math.min(1, state.throttle + 0.05); state.enginesRunning = true; break;
      case '-': case '_': state.throttle = Math.max(0, state.throttle - 0.05); break;
      ...
      case 'escape': state.pitchInput = 0; state.rollInput = 0; state.yawInput = 0; break;
    }
  ...
  private handleBlur = () => {
    this.pressed.clear();
    const state = this.getState(); if (!state) return;
    state.pitchInput = 0; state.rollInput = 0; state.yawInput = 0; state.brakesActive = false;
  };
